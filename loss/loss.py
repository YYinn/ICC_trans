...
class BCEFocalLoss(torch.nn.Module):
    def __init__(self, gamma=0, alpha=0.8, reduction='elementwise_mean'): ###
        super().__init__()
        self.gamma = gamma
        self.alpha = alpha
    ...
    def forward(self, _input, target):
        ...
        pt = _input
        alpha = self.alpha
        loss = - alpha * (1 - pt) * target * torch.log(pt) - (1 - alpha) * pt * (1 - target) * torch.log(1 - pt)
        # loss = - alpha ** (1 - pt)  * target * torch.log(pt) -  (1 - target) * torch.log(1 - pt)
        # loss = - alpha ** (1 - pt)  * target -  (1 - target)
