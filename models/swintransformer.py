...
# model = SwinNet1(img_size=(args.roi_x, args.roi_y, args.roi_z),
...
class mymodel(nn.Module):
    def __init__(
        ...
        self._fc1 = nn.Sequential(nn.Linear(32768, 512), nn.ReLU())
    ...
    def forward(self, multimods):
        m1_f = self.swinViT(multimods)
        # m2_f = self.swintrans(multimods[1])
        # m3_f = self.swintrans(multimods[2])
        # m4_f = self.swintrans(multimods[3])
        # m5_f = self.swintrans(multimods[4])
        # m6_f = self.swintrans(multimods[5])
        # m7_f = self.swintrans(multimods[6])
        print(multimods.shape, m1_f[0].shape, m1_f[1].shape, m1_f[2].shape, m1_f[3].shape, m1_f[4].shape)
        f1 = self.fl(m1_f[0])
        print(f1.shape)
        # f = torch.cat([m1_f, m2_f, m3_f, m4_f, m5_f, m6_f])
        h = self._fc1(f1) #[B, n, 512]
