...
class WindowAttention(nn.Module):
    ...
    def forward(self, x, mask=None):
        ...
        relative_position_bias = self.relative_position_bias_table[
            self.relative_position_index.clone()[:N, :N].reshape(-1)].reshape(N, N, -1)  # Wh*Ww,Wh*Ww,nH
...
class SwinTransformerBlock(nn.Module):
    ...
    def forward_part1(self, x, mask_matrix):
        ...
        if len(x.shape) == 5:
            ...
            x = nn.functional.pad(x, (0, 0, pad_l, pad_r, pad_t, pad_b, pad_d0, pad_d1))
        ...
        # output dim nW*B, window_size * window_size (or window_size * window_size * window_size), C

        # W-MSA/SW-MSA
        attn_windows = self.attn(x_windows, mask=attn_mask)
        # nW*B, window_size*window_size (or window_size * window_size * window_size), C

        # merge windows
        attn_windows = attn_windows.view(-1, *(window_size + (C,)))
        shifted_x = window_reverse(attn_windows, window_size, dims=dims)  # B D H' W' C or B H' W' C
...
class BasicLayer(nn.Module):
    ...
    def __init__(self, dim, depth, num_heads, window_size,
        ...
        self.blocks = nn.ModuleList([
            SwinTransformerBlock(dim=self.dim, num_heads=num_heads,
...
class SwinTransformer(nn.Module):
    ...
    def __init__(self, img_size, patch_size, in_chans, num_classes, embed_dim, depths, num_heads, window_size,
                 mlp_ratio=4., qkv_bias=True, qk_scale=None, drop_rate=0., attn_drop_rate=0., drop_path_rate=0.1,
                 norm_layer=nn.LayerNorm, ape=False, patch_norm=True, use_checkpoint=False, spatial_dims=3, **kwargs):
        super().__init__()
